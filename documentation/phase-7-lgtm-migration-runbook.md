...
> **Why the `offset 30s`.** Pie charts want an **instant** query (one value
> per series), not a **range** query (a time series that the pie then
> reduces). But an instant query evaluated at "now" can race with Loki's
> ingestion lag — the 5-minute window may end slightly past the most
> recently flushed chunk and return empty for some series. The result:
> slices disappear intermittently. A 30-second offset shifts the evaluation
> point safely inside flushed data, making all series appear reliably. The
> panel's `queryType: instant` is paired with this offset; do not set one
> without the other.
...
| 3 | Sensor Trend Distribution | Pie chart | Loki | Panel 3 query above (keep the `offset 30s`) | Query options → Type: **Instant**. Legend: `{{Trend}}` |
